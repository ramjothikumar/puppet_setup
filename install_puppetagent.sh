...
puppet_master_fqdn="$1"
if [ "$puppet_master_fqdn" == "" ]; then
    echo "Usage: install_puppetagent.sh <puppet_master_fqdn>"
    echo "Puppet Master FQDN not Provided. Exiting"
    exit 0
fi
echo "Puppet Master IP: $puppet_master_fqdn"
...
post_install_steps()
{
    ...
    echo "server = $puppet_master_fqdn" | $sudo tee --append /etc/puppet/puppet.conf
